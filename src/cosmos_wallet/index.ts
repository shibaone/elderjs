...
async function cosmos_getElderClient(
    elderChainConfig: ElderConfig,
    opts?: { signer?: OfflineSigner }
) {
    ...
    try {
        if (!offlineSigner)
            offlineSigner = await getKeplrSigner(elderChainInfo);
        ...
        let { elderAccountNumber, elderAccountSequence } =
            await getAccountNumberAndSequence(
                elderChainConfig.rest,
                elderAddress
            );
...
async function cosmos_getElderMsgAndFeeTxRaw(
    ...
    const offlineSigner = (opts?.signer ||
        (await getKeplrSigner(elderChainInfo))) as OfflineDirectSigner;
